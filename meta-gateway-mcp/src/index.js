...
import { MetaAdapter, MetaResult } from './adapters/base.js';
dotenv.config();
...
if (process.env.ORACLE_USER) {
    adapters.push(new OracleAdapter(process.env.ORACLE_USER, process.env.ORACLE_PASSWORD || '', process.env.ORACLE_CONNECTION_STRING || ''));
}
if (process.env.PG_CONNECTION_STRING) {
    adapters.push(new PostgresAdapter(process.env.PG_CONNECTION_STRING));
}
if (process.env.ES_NODE) {
    adapters.push(new ElasticsearchAdapter(process.env.ES_NODE, process.env.ES_API_KEY));
}
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: 'search_meta',
                description: 'Search corporate metadata across Oracle, PostgreSQL, Elasticsearch, and Static Vector DB.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        query: {
                            type: 'string',
                            description: "The business term or technical name to search for (e.g., '고객 등급', 'TB_USER')",
                        },
                        source: {
                            type: 'string',
                            enum: ['all', 'oracle', 'postgres', 'elasticsearch', 'vector'],
                            description: 'Specific source to search from (optional, defaults to all)',
                        },
                    },
                    required: ['query'],
                },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === 'check_connections') {
        const status = await Promise.all(adapters.map(async (a) => ({
            name: a.name,
    ...
    if (name === 'search_meta') {
        ...
        const activeAdapters = sourceFilter === 'all'
            ? adapters
        ...
        const results = await Promise.all(activeAdapters.map((a) => a.search(query)));
